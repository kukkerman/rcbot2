...
public:
    ...
    eBotAction getId() const;
    bool canDo() const;
    CBotWeapon *getWeaponChoice() const;
...
public:
    void freeMemory();
    void addUtility(CBotUtility p);
	void execute();

    CBotUtility* nextBest();

private:
    std::vector<CBotUtility> utilities;
    std::vector<CBotUtility>::iterator best;
};
